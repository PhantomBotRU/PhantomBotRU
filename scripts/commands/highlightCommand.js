    ...
    $.bind('command', function(event) {
        ...
        if (command.equalsIgnoreCase('highlight')) {
            if (!$.isOnline($.channelName)) {
                ...
                $.say($.whisperPrefix(sender) + $.lang.get('highlightcommand.highlight.usage', $.inidb.exists('settings', 'timezone') ? $.inidb.get('settings', 'timezone') : 'Europe/Moscow'));
                return;
            }
        ...
        if (command.equalsIgnoreCase('gethighlights') || command.equalsIgnoreCase('showhighlights')) {
            ...
            for (var i in keys) {
                temp.push('[отметка ' + keys[i] + ' ' + $.inidb.get('highlights', keys[i]) + '] ');
            }
