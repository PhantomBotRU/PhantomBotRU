...
(function() {
    var levelWithTime = $.getSetIniDbBoolean('timeSettings', 'timeLevel', false),
        ...
        hoursForLevelUp = $.getSetIniDbNumber('timeSettings', 'timePromoteHours', 240),
        regularsGroupId = 6,
        interval,
        inter;
     ...
     *
     *     getCurLocalTimeString("dd MMMM yyyy hh:mm:ss zzz '('Z')'");
     */
    function getCurLocalTimeString(format) {
        var dateFormat = new java.text.SimpleDateFormat(format);
        dateFormat.setTimeZone(java.util.TimeZone.getTimeZone(($.inidb.exists('settings', 'timezone') ? $.inidb.get('settings', 'timezone') : "Europe/Moscow")));
     ...
     */
    function getLocalTime() {
        var dateFormat = new java.text.SimpleDateFormat("dd-MM-yyyy'T'HH:mm:ss");
        dateFormat.setTimeZone(java.util.TimeZone.getTimeZone(($.inidb.exists('settings', 'timezone') ? $.inidb.get('settings', 'timezone') : "Europe/Moscow")));
        return dateFormat.format(new java.util.Date());
    }
    ...
    $.bind('command', function(event) {
        ...
        if (command.equalsIgnoreCase('time')) {
            if (!action) {
                ...
                /**
                 * @commandpath time autolevel - Auto levels a user to regular after hitting 240 hours.
                 */
        ...
        if (command.equalsIgnoreCase('streamertime')) {
            $.say($.whisperPrefix(sender) + $.lang.get('timesystem.streamertime', getCurLocalTimeString("dd MMMM',' hh:mm a '('zzz')'"), $.username.resolve($.ownerName)));
        }
        ...
        if (command.equalsIgnoreCase('timezone')) {
            ...
            if (!action) {
                $.say($.whisperPrefix(sender) + $.lang.get('timesystem.set.timezone.usage', ($.inidb.exists('settings', 'timezone') ? $.inidb.get('settings', 'timezone') : "Europe/Moscow")));
                return;
            }

            tzData = java.util.TimeZone.getTimeZone(action);
            if (tzData.getID().equals("Europe/Moscow") && !action.equals("Europe/Moscow")) {
