...
(function() {
    var playlistDbPrefix = 'ytPlaylist_',
        ...
        announceInChat = $.getSetIniDbBoolean('ytSettings', 'announceInChat', true),
        ...
        songRequestsMaxParallel = $.getSetIniDbNumber('ytSettings', 'songRequestsMaxParallel', 5),
        songRequestsMaxSecondsforVideo = $.getSetIniDbNumber('ytSettings', 'songRequestsMaxSecondsforVideo', (8 * 60)),
        stealRefund = $.getSetIniDbBoolean('ytSettings', 'stealRefund', true),
    ...
    function reloadyt() {
        ...
        stealRefund = $.getIniDbBoolean('ytSettings', 'stealRefund', true);
        voteCount = $.getIniDbNumber('ytSettings', 'voteCount', 5);
    ...
    function YoutubeVideo(searchQuery, owner) {
        ...
        if ($.inidb.exists('ytcache', searchQuery)) {
            ...
            do {
                ...
                attempts++;
            } while (data[0].length() < 11 && data[1] != "Совпадений не найдено" && attempts <= 2);
        ...
        if (license == 0 && playCCOnly) {
            throw 'трек с ID ' + videoId + ' не имеет свободной лицензии Creative Commons';
        }
        if (embeddable == 0) {
            throw 'трек с ID ' + videoId + ' запрещён к воспроизведению на сторонних ресурсах';
        }
