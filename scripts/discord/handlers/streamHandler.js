...
(function() {
    var onlineToggle = $.getSetIniDbBoolean('discordSettings', 'onlineToggle', false),
        onlineMessage = $.getSetIniDbString('discordSettings', 'onlineMessage', 'Запуск стрима (name)'),
        offlineToggle = $.getSetIniDbBoolean('discordSettings', 'offlineToggle', false),
        offlineMessage = $.getSetIniDbString('discordSettings', 'offlineMessage', 'Завершение стрима (name)'),
        gameToggle = $.getSetIniDbBoolean('discordSettings', 'gameToggle', false),
        gameMessage = $.getSetIniDbString('discordSettings', 'gameMessage', 'Смена игры на стриме (name)'),
    ...
    $.bind('webPanelSocketUpdate', function(event) {
        if (event.getScript().equalsIgnoreCase('./discord/handlers/streamHandler.js')) {
            ...
            onlineMessage = $.getIniDbString('discordSettings', 'onlineMessage', 'Запуск стрима (name)');
            offlineToggle = $.getIniDbBoolean('discordSettings', 'offlineToggle', false);
            offlineMessage = $.getIniDbString('discordSettings', 'offlineMessage', 'Завершение стрима (name)');
            gameToggle = $.getIniDbBoolean('discordSettings', 'gameToggle', false);
            gameMessage = $.getIniDbString('discordSettings', 'gameMessage', 'Смена игры на стриме (name)');
    ...
    function getTrimmedGameName() {
        ...
        return (game.length > 18 ? game.substr(0, 17) + '…' : game);
    }
