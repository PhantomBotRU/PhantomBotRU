...
(function() {
    var blacklist = [],
        ...
        cbenniToggle = $.getSetIniDbBoolean('discordSettings', 'cbenniToggle', true);
    ...
    function embedDelete(username, creator, message) {
        ...
        obj['**Удаление**' + '\r\nПользователь:'] = '[' + username + '](https://twitch.tv/' + username.toLowerCase() + ')';
        obj['Последнее сообщение:'] = (message.length() > 500 ? message.substring(0, 500) + '…' : message);
        obj['Инициатор:'] = creator;
    ...
    function embedTimeout(username, creator, reason, time, message) {
        ...
        obj['**Таймаут**' + '\r\nПользователь:'] = '[' + username + '](https://twitch.tv/' + username.toLowerCase() + ')';
        obj['Продолжительность:'] = time + ' сек';
        obj['Причина:'] = reason;
        obj['Последнее сообщение:'] = (message.length() > 500 ? message.substring(0, 500) + '…' : message);
        obj['Инициатор:'] = creator;

        if (cbenniToggle) {
            obj['Лог:'] = '[https://cbenni.com/' + $.channelName + '?user=' + username.toLowerCase() + '](https://cbenni.com/' + $.channelName + '?user=' + username.toLowerCase() + ')';
        }
    ...
    $.bind('PubSubModerationUnTimeout', function(event) {
        ...
        $.discordAPI.sendMessageEmbed(modLogChannel, 'green', '**Амнистия**' + '\r\nВид: снятие таймаута' + '\r\nПользователь: ' + '[' + username + '](https://twitch.tv/' + username.toLowerCase() + ')' + '\r\nИнициатор: ' + creator);
    });
    ...
    $.bind('discordChannelCommand', function(event) {
        ...
        if (command.equalsIgnoreCase('moderation')) {
            ...
            if (action.equalsIgnoreCase('longmessage')) {
                if (subAction === undefined) {
                    $.discord.say(channel, $.discord.userPrefix(mention) + $.lang.get('moderation.long.msg.usage'));
                    return;
                }
                ...
                if (subAction.equalsIgnoreCase('toggle')) {
                    ...
                    $.discord.say(channel, $.discord.userPrefix(mention) + $.lang.get('moderation.long.msg.toggle', (longMessageToggle === true ? $.lang.get('common.enabled') : $.lang.get('common.disabled'))));
                }
                ...
                if (subAction.equalsIgnoreCase('limit')) {
                    if (isNaN(parseInt(actionArgs))) {
                        $.discord.say(channel, $.discord.userPrefix(mention) + $.lang.get('moderation.long.msg.limit.usage'));
                        return;
                    }

                    longMessageLimit = parseInt(actionArgs);
                    $.setIniDbNumber('discordSettings', 'longMessageLimit', longMessageLimit);
                    $.discord.say(channel, $.discord.userPrefix(mention) + $.lang.get('moderation.long.msg.limit.set', longMessageLimit));
                }
            }
