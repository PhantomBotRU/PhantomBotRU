...
(function() {
    var raidToggle = $.getSetIniDbBoolean('raidSettings', 'raidToggle', true),
        newRaidIncMessage = $.getSetIniDbString('raidSettings', 'newRaidIncMessage', '(name) зарейдил(а) канал с (viewers) зрителями'),
        raidIncMessage = $.getSetIniDbString('raidSettings', 'raidIncMessage', '(name) зарейдил(а) канал (times)-й раз с (viewers) зрителями'),
        raidReward = $.getSetIniDbNumber('raidSettings', 'raidReward', 100),
        raidOutMessage = $.getSetIniDbString('raidSettings', 'raidOutMessage', 'Рейдим (name) на канале (url)'),
        raidOutSpam = $.getSetIniDbNumber('raidSettings', 'raidOutSpam', 3);
    ...
    function handleOutRaid(username) {
        ...
        if (message.match(/\(name\)/)) {
            message = $.replace(message, '(name)', $.username.resolve(username));
        }
