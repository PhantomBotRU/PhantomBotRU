 ...
 */
(function() {
    var subMessage = $.getSetIniDbString('subscribeHandler', 'subscribeMessage', '(name) оформил(а) подписку'),
        primeSubMessage = $.getSetIniDbString('subscribeHandler', 'primeSubscribeMessage', '(name) оформил(а) прайм-подписку'),
        reSubMessage = $.getSetIniDbString('subscribeHandler', 'reSubscribeMessage', '(name) оформил(а) подписку в (months)-й раз подряд (customemote)'),
        giftSubMessage = $.getSetIniDbString('subscribeHandler', 'giftSubMessage', '(name) подарил(а) (recipient) подписку'),
        giftAnonSubMessage = $.getSetIniDbString('subscribeHandler', 'giftAnonSubMessage', 'Анонимный пользователь подарил (recipient) подписку'),
        massGiftSubMessage = $.getSetIniDbString('subscribeHandler', 'massGiftSubMessage', '(name) подарил(а) (amount) подписок случайным пользователям'),
        massAnonGiftSubMessage = $.getSetIniDbString('subscribeHandler', 'massAnonGiftSubMessage', 'Анонимный пользователь подарил (amount) подписок случайным пользователям'),
        ...
        subReward = $.getSetIniDbNumber('subscribeHandler', 'subscribeReward', 500),
        reSubReward = $.getSetIniDbNumber('subscribeHandler', 'reSubscribeReward', 500),
        giftSubReward = $.getSetIniDbNumber('subscribeHandler', 'giftSubReward', 500),
        massGiftSubReward = $.getSetIniDbNumber('subscribeHandler', 'massGiftSubReward', 500),
        customEmote = $.getSetIniDbString('subscribeHandler', 'resubEmote', 'PurpleStar'),
        subPlan1000 = $.getSetIniDbString('subscribeHandler', 'subPlan1000', '1-й уровень'),
        subPlan2000 = $.getSetIniDbString('subscribeHandler', 'subPlan2000', '2-й уровень'),
        subPlan3000 = $.getSetIniDbString('subscribeHandler', 'subPlan3000', '3-й уровень'),
        announce = false,
        emotes = [],
        i;
    ...
    function updateSubscribeConfig() {
        ...
        subPlan1000 = $.getIniDbString('subscribeHandler', '1-й уровень');
        subPlan2000 = $.getIniDbString('subscribeHandler', '2-й уровень');
        subPlan3000 = $.getIniDbString('subscribeHandler', '3-й уровень');
    }
    ...
    $.bind('twitchMassSubscriptionGifted', function(event) {
        ...
        if (massGiftSubWelcomeToggle === true && announce === true) {
            ...
            */
            ...
            if (amount % 100 == 11) {
                s = $.replace(s, 'подписок', 'подписок');
            }

            if (amount % 10 == 1) {
                s = $.replace(s, 'подписок', 'подписку');
            }

            if (amount % 100 >= 12 && amount % 100 <= 14) {
                s = $.replace(s, 'подписок', 'подписок');
            }

            if (amount % 10 >= 2 && amount % 10 <= 4) {
                s = $.replace(s, 'подписок', 'подписки');
            }
