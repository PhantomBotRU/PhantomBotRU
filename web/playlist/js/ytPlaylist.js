...
connection.onmessage = function(e) {
    ...
    if (messageObject['authresult'] === false) {
        if (!messageObject['authresult']) {
            newAlert('Аутентификация не удалась', 'Аутентификация не удалась', 'danger', 0);
            return;
        }
        return;
    }
...
function handleNewSong(title, duration, requester, id) {
    ...
    $('#currentSongTable').html('<tr><th width=\"30px\">&nbsp;</th><th>Трек</th><th width=\"120px\">Заказчик</th><th width=\"60px\">Длит.</th><th width=\"110px\">YouTube ID</th></tr>' +
                                '<tr><td>\u266B</td><td>' + title + '</td><td>' + requester + '</td><td>' + duration + '</td><td>' + id + '</td></tr>');
}

function handlePlayList(d) {
    debugMsg('handlePlayList(' + d + ')');
    $('#playlistTableTitle').html('Плейлист ' + d['playlistname']);
    var tableData = '<tr><th width=\"30px\">№</th><th>Трек</th><th width=\"60px\">Длит.</th><th width=\"110px\">YouTube ID</th></tr>';
    for (var i in d['playlist']) {
        var id = d['playlist'][i]['song'];
        var title = d['playlist'][i]['title'];
        var duration = d['playlist'][i]['duration'];
        tableData += '<tr><td>' + (parseInt(i) + 1) + '</td><td>' + title + '</td><td>' + duration + '</td><td>' + id + '</td></tr>';
    }
...
function handleSongList(d) {
    ...
    var tableData = '<tr><th width=\"30px\">№</th><th>Трек</th><th width=\"120px\">Заказчик</th><th width=\"60px\">Длит.</th><th width=\"110px\">YouTube ID</th></tr>';
    for (var i in d['songlist']) {
        ...
        tableData += '<tr><td>' + (parseInt(i) + 1) + '</td><td>' + title + '</td><td>' + requester + '</td><td>' + duration + '</td><td>' + id + '</td></tr>';
    }
