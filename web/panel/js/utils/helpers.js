...
$(function() {
    ...
    helpers.fixNumber = function(number, force) {
        ...
        if (!isNaN(newNumber)) {
            if (newNumber > thousandReplace && newNumber < millionReplace) {
                return ((newNumber / 1000).toFixed(1) + ' тыс.');
            } else if (newNumber > millionReplace) {
                return ((newNumber / 1000000).toFixed(1) + ' млн.');
            }
        }
    ...
    helpers.getEventMessage = function(event) {
        switch (event.type.toLowerCase()) {
            case 'подписка':
                return ('от ' + event.username);
            case 'прайм-подписка':
                return ('от ' + event.username);
            case 'мультиподписка':
                return ('от ' + event.username + ' в ' + event.months + '-й раз подряд');
            case 'фолловинг':
                return ('от ' + event.username);
            case 'биты':
                return (event.amount + ' шт. от ' + event.username);
            case 'хост':
                return ('от ' + event.username + ' на ' + event.viewers + ' чел.');
            case 'автохост':
                return ('от ' + event.username + ' на ' + event.viewers + ' чел.');
            case 'донат': // To be added soon.
                break;
            case 'рейд':
                return ('от ' + event.username + ' с ' + event.viewers + ' чел.');
            case 'подписка&nbsp;в&nbsp;подарок':
                return (event.recipient + ' от ' + event.username);
        }
    };
    ...
    helpers.handleInputString = function(obj) {
        ...
        if (obj.val().length < 1) {
            if (!obj.parent().hasClass('has-error')) {
                ...
                obj.after($('<p/>', {
                    'class': 'help-block',
                    'text': 'Данное поле не может оставаться пустым'
                }));
    ...
    helpers.handleInputNumber = function(obj, min, max) {
        ...
        if (isNaN(parseInt(obj.val())) || isNaN(obj.val()) || parseInt(obj.val()) < min || parseInt(obj.val()) > newMax) {
            if (!obj.parent().hasClass('has-error')) {
                ...
                obj.after($('<p/>', {
                    'class': 'help-block',
                    'text': 'Число должно быть не менее ' + min + (max !== undefined ? ' и не более ' + newMax + '' : '')
                }));
                let btn = obj.closest('form').find('button');
                if (btn.data('candisable') !== undefined) {
                    // Disable the button
    ...
    helpers.handleInputDate = function(obj) {
        ...
        if (matched === null || ((matched[6].length < 4 && matched[2].length == 2) || (matched[6].length == 2 && matched[2].length < 4))) {
            if (!obj.parent().hasClass('has-error')) {
                ...
                toastr.error('Введена некорректная дата');
                return false;
            }
        } else {
            if (obj.parent().find('p').length > 0) {
    ...
    helpers.getAdvanceModal = function(id, title, btn, body, onClose) {
        return $('<div/>', {
            ...
            $(this).find('.collapse-btn').html('Меньше настроек');
        }).on('hide.bs.collapse', function() {
            $(this).find('.glyphicon').removeClass('glyphicon-chevron-up').addClass('glyphicon-chevron-down');
            $(this).find('.collapse-btn').html('Больше настроек');
        });
    };
     ...
     * @return {Object}
     */
    helpers.getInputGroup = function(id, type, title, placeholder, value, toolTip, disabled, min) {
        return $('<div/>', {
            'class': 'form-group'
        }).append($('<lable/>', {
            'html': $('<b/>', {
    ...
    /*
     * @function gets a sub-comment.
     *
     * @param  {String}  text
     * @return {Object}
     */
    helpers.getSubComment = function(text) {
        return $('<div/>', {
            'style': 'font-size: 80%; margin-top: 2px !important; opacity: 0.65 !important'
        }).append($('<span/>', {
            'text': text
        }));
    };
    ...
    /*
     * @function gets a note.
     *
     * @param  {String}  text
     * @return {Object}
     */
    helpers.getNote = function(text) {
        return $('<div/>', {
            'style': 'font-size: 90%; color: #f39c12 !important'
        }).append($('<span/>', {
            'text': text
        }));
    };
    ...
    helpers.getConfirmDeleteModal = function(id, title, hasBodyMsg, closeMessage, onClose) {
        swal({
            'title': title,
            'text': (hasBodyMsg ? 'Это действие необратимо' : ''),
            'icon': 'warning',
            'reverseButtons': true,
            'buttons': {
                'confirm': {
                ...
                $('#notifications-menu-ul').append($('<li/>').append($('<a/>', {
                    ...
                    'click': function() {
                        ...
                        .append($('<p/>', {
                            ...
                            '<b>Please check ' +
                                $('<a/>', { 'target': '_blank' }).prop('href', 'https://community.phantom.bot/t/how-to-update-phantombot').append('this guide')[0].outerHTML +
