...
$(run = function() {
    ...
    socket.getDBTableValues('get_all_modules', 'modules', function(results) {
        ...
        for (let i = 0; i < twitchModules.length; i++) {
            twitchTable.push([
                ...
                $('<div/>', {
                    ...
                    'data-toggle': 'tooltip',
                    'title': (twitchModules[i].status ? 'Выключить модуль' : 'Включить модуль') ,
                    'style': 'float: right',
                    'data-module': twitchModules[i].module,
        ...
        table.on('click', '.btn-danger', function() {
            ...
            helpers.getConfirmDeleteModal('rm_module_cmd_modal', 'Вы уверены, что хотите удалить модуль Twitch «' + module + '»?', true,
                    'Модуль Twitch «' + module + '» успешно удалён', function() {
        ...
        socket.getDBValue('get_discord_status', 'panelData', 'hasDiscord', function(e) {
            ...
            for (let i = 0; i < discordModules.length; i++) {
                discordTable.push([
                    ...
                    $('<div/>', {
                        ...
                        'data-toggle': 'tooltip',
                        'title': (discordModules[i].status ? 'Выключить модуль' : 'Включить модуль') ,
                        'style': 'float: right',
                        'data-module': discordModules[i].module,
            ...
            table.on('click', '.btn-danger', function() {
                ...
                // Ask the user if he want to remove the module.
                helpers.getConfirmDeleteModal('rm_module_cmd_modal', 'Вы уверены, что хотите удалить модуль «' + module + '»?', true,
                        'Модуль «' + module + '» успешно удалён', function() {
