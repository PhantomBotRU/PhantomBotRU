...
$(function() {
    ...
    // Convert digit grouping separator from comma to space for some countries.
    ...
    socket.getDBValues('panel_get_settings', {
        ...
        socket.getDBValue('dashboard_get_events', 'panelData', 'data', function(e) {
            ...
            socket.getDBValue('dashboard_get_data', 'panelData', 'stream', function(e) {
                if (e.panelData === null) {
                    alert('Боту необходимо собрать данные, необходимые для загрузки интерфейса. Перезагрузите страницу позднее.');
                    return;
                }
                ...
                socket.getDBTableValues('dashboard_get_commands', 'command', function(e) {
                    ...
                    if (helpers.DEBUG_STATE === helpers.DEBUG_STATES.DEBUG) {
                        ...
                        $(document).ready(function() {
                            ...
                            helpers.handlePanelSetInfo($('#dashboard-views').data('number', commaToSpace(helpers.parseNumber(tempData.views))), 'dashboard-views', helpers.fixNumber(tempData.views));
                            // Set viewers.
                            helpers.handlePanelSetInfo($('#dashboard-viewers').data('number', commaToSpace(helpers.parseNumber(tempData.viewers))), 'dashboard-viewers', helpers.fixNumber(tempData.viewers));
                            // Set followers.
                            helpers.handlePanelSetInfo($('#dashboard-followers').data('number', commaToSpace(helpers.parseNumber(tempData.followers))), 'dashboard-followers', helpers.fixNumber(tempData.followers));
                        });
                    } else {
...
$(function() {
    ...
    $('#dashboard-btn-update').on('click', function() {
        ...
        socket.sendCommand('update_title', 'settitlesilent ' + $('#stream-title').val(), function() {
            ...
            socket.sendCommand('update_game', 'setgamesilent ' + $('#stream-game').val(), function() {
                toastr.success('Информация о стриме успешно обновлена');
            });
        });
    });
    ...
    $('.user-action').on('click', function() {
        ...
        socket.sendCommand('user_action_cmd', command, function() {
            ...
            // Let the user know.
            toastr.success('Действие для ' + username + ' успешно запущено');
        });
    });
    ...
    $('#dashboard-btn-instant-commercial').on('click', function() {
        if ($('#instant-commercial-length').val() === "") {
            toastr.error('Не выбрана длительность рекламы');
            return;
        }
    ...
    helpers.setInterval(function() {
        ...
        socket.getDBValue('dashboard_get_data_refresh', 'panelData', 'stream', function(e) {
            ...
            e = JSON.parse(e.panelData);
            // Set views if not hidden.
            helpers.handlePanelSetInfo($('#dashboard-views').data('number', commaToSpace(helpers.parseNumber(e.views))), 'dashboard-views', helpers.fixNumber(e.views));
            // Set viewers.
            helpers.handlePanelSetInfo($('#dashboard-viewers').data('number', commaToSpace(helpers.parseNumber(e.viewers))), 'dashboard-viewers', helpers.fixNumber(e.viewers));
            // Set followers.
            helpers.handlePanelSetInfo($('#dashboard-followers').data('number', commaToSpace(helpers.parseNumber(e.followers))), 'dashboard-followers', helpers.fixNumber(e.followers));
            // Set uptime.
            if (e.isLive) {
                $('#dashboard-uptime').html(e.uptime);
                $('#bg-uptime').removeClass('bg-red').addClass('bg-green');
