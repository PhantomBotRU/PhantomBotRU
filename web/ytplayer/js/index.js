...
$(function() {
    ...
    const openPlayer = hasPlaylistData => {
        ...
        if (!hasPlaylistData && player.hasAPIKey() && !player.secondConnection) {
            toastr.error('Не удалось загрузить плейлист');
            ...
            // Error the to user.
            helpers.getErrorModal('Ошибка плейлиста', 'Не удалось загрузить плейлист', () => {
                player.dbQuery('get_playlists', 'yt_playlists_registry', (results) => {
                    ...
                    helpers.getPlaylistModal('Загрузка плейлиста', 'Имя', 'Загрузить', 'Плейлист', playlists, () => {
                        let playlist = $('#playlist-load').find(':selected').text();

                        if (playlist === 'Select a playlist') {
                            toastr.error('Некорректный формат плейлиста');
                        } else {
                            if (playlist.length > 0) {
                                player.loadPlaylist(playlist);
                                toastr.success('Плейлист «' + playlist + '» загружен');
    ...
    window.onYouTubeIframeAPIReady = () => {
        ...
        player.dbQuery('get_module_status', 'modules', (data) => {
            if (data['./systems/youtubePlayer.js'] == 'false') {
                helpers.getErrorModal('Модуль выключен', 'Для работы плейера необходимо включить его модуль', () => {
        ...
        player.addListener('play', (e) => {
            ...
            if (player.firstLoad === true) {
                ...
                openPlayer(true);
                // Alert the user.
                toastr.info('Трек «' + (e.title.length > 76 ? e.title.substring(0, 75) + '…' : e.title + '» находится в ожидании'));
            } else {
                player.API.loadVideoById(e.play, 0, 'medium');
                toastr.success('Звучит трек «' +  (e.title.length > 76 ? e.title.substring(0, 75) + '…' : e.title + '»'));
            }
...
$(function() {
    ...
    $('#del-cur-playlist-button').on('click', () => {
        ...
        toastr.success('Трек «' + (player.temp.title.length > 76 ? player.temp.title.substring(0, 75) + '…' : player.temp.title) + '» удалён из плейлиста');
    });

    // Fav button to steal a song.
    $('#fav-button').on('click', () => {
        player.addSongToPlaylist();
        toastr.success('Трек «' + (player.temp.title.length > 76 ? player.temp.title.substring(0, 75) + '…' : player.temp.title) + '» добавлен в плейлист');
    });
    ...
    $('#queue-add-song-button').on('click', () => {
        helpers.getSongModal('Добавление трека в очередь', 'Ссылка', 'Добавить', 'Ссылка на трек в YouTube', () => {
    ...
    $('#playlist-add-song-button').on('click', () => {
        helpers.getSongModal('Добавление трека в плейлист', 'Ссылка', 'Добавить', 'Ссылка на трек в YouTube', () => {
            let song = $('#song-url').val();
    ...
    $('#load-playlist-button').on('click', () => {
        player.dbQuery('get_playlists', 'yt_playlists_registry', (results) => {
            ...
            helpers.getPlaylistModal('Загрузка плейлиста', 'Плейлист', 'Загрузить', 'Имя плейлиста', playlists, () => {
                let playlist = $('#playlist-load').find(':selected').text();

                if (playlist === 'Выберите плейлист') {
                    toastr.error('Необходимо выбрать корректный плейлист');
                } else {
                    if (playlist.length > 0) {
                        player.loadPlaylist(playlist);
                        toastr.success('Плейлист «' + playlist + '» загружен');
