...
$(function() {
    ...
    socket.onclose = (e) => {
        ...
        if (secondConnection) {
            toastr.error('Соединение с ботом прервано', '', {timeOut: 0});
        } else {
            toastr.error('Необходимо убедиться в наличии связи с ботом, его функционировании и перезагрузить страницу', '', {timeOut: 0});
        }
    };
        ...
        try {
            ...
            if (message.secondconnection !== undefined) {
                if (message.secondconnection === true) {
                    ...
                    toastr.error('Плейер уже открыт в другом окне, бот закрыл соединение', '',
            ...
            if (message.ytkeycheck !== undefined) {
                if (message.ytkeycheck === false) {
                    ...
                    toastr.error('Необходимо получить ключ в ' +
                                 '<a href="https://console.developers.google.com">консоли разработчика Google' +
                                 '</a>', 'Отсутствует ключ YouTube API',
                                 {timeOut: 0, extendedTimeOut: 0});
                }
                return;
            }
